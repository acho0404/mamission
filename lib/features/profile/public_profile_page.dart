...
class _PublicProfilePageState extends State<PublicProfilePage> {
  ...
  String _formatNameShort(String fullName) {
    ...
    final parts =
    fullName.trim().replaceAll(RegExp(r'\s+'), ' ').split(' ');
    if (parts.length <= 1) return parts[0];
    return "${parts[0]} ${parts[1][0].toUpperCase()}.";
  }

  Future<String?> _pickAndUpload(String folderName,
    ...
    try {
      ...
      Reference ref = _storage
          .ref()
          .child('users/${widget.userId}/$folderName/$fileName');
  ...
  Future<void> _saveProfile(
      String uid, bool isPro, StateSetter setSheetState) async {
    // 1. Validation Ville
    if (_cityInputCtrl.text.isNotEmpty && !_isCitySelectedFromList) {
      setSheetState(() {
        _cityErrorText =
        "Veuillez sélectionner une ville dans la liste suggérée.";
      });
      return;
    } else {
  ...
  void _openEditSheet(Map<String, dynamic> data) {
    ...
    _tempPortfolioUrls =
        (data['portfolio'] as List?)?.cast<String>() ?? [];
    _currentSkills =
        (data['skills'] as List?)?.cast<String>() ?? [];
    _currentEquipments =
        (data['equipments'] as List?)?.cast<String>() ?? [];
                                  ...
                                  for (int i = 0; i < 3; i++)
                                    _PortfolioUploadSlot(
                                      url: i <
                                          _tempPortfolioUrls
                                              .length
                                          ? _tempPortfolioUrls[i]
                                          : null,
                                      isLoading:
                                      _uploadingPortfolioIndex ==
                                          i,
                                      onTap: () async {
                                        FocusScope.of(
                                            sheetContext)
    ...
    return Scaffold(
          ...
          if (isOwner)
            Padding(
              ...
              child: IconButton(
                ...
                onPressed: () =>
                    _db.collection('users').doc(widget.userId).get().then(
                          (doc) {
      ...
      body: StreamBuilder<
        ...
        builder: (context, snapshot) {
          ...
          final tagline =
          (data['tagline'] ?? 'Membre MaMission') as String;
          ...
          if (isOwner &&
              widget.openEditOnStart &&
              !_didOpenInitialEdit) {
            _didOpenInitialEdit = true;
            WidgetsBinding.instance.addPostFrameCallback(
                    (_) => _openEditSheet(data));
          }
          ...
          return SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                      ...
                      children: [
                        ...
                        Stack(
                          ...
                          children: [
                            ...
                            // 🔥 Badge visibilité type Insta (lié à l’abonnement)
                            if (subActive)
                              Container(
                                ...
                                child: Row(
                                  ...
                                  children: [
                                    Icon(
                                      ...
                                      size: 14,
                                      color: isProActive
                                          ? const Color(0xFFFFC107) // PRO doré
                                          : kPrimary,               // Standard en violet MaMission
                                    ),
                                    ...
                                    Text(
                                      ...
                                      style: TextStyle(
                                        ...
                                        color: isProActive ? kPrimary : Colors.black87,
                                      ),
                                    ),
                                  ],
                                ),
                        ...
                        FutureBuilder<Map<String, int>>(
                          ...
                          builder: (ctx, snap) {
                            ...
                            postedCount > 0
                                ? "${((completedCount / postedCount) * 100).round()}%"
                                : "N/A";
                            return Padding(
                ...
                // CTA haut de page pour clients (pas le propriétaire)
                if (!isOwner)
                  Padding(
                    padding:
...
class _ZoneInterventionSelector extends StatefulWidget {
  ...
  @override
  State<_ZoneInterventionSelector> createState() =>
      _ZoneInterventionSelectorState();
}

class _ZoneInterventionSelectorState
    extends State<_ZoneInterventionSelector> {
...
class _SmartTagSection extends StatefulWidget {
  ...
  @override
  State<_SmartTagSection> createState() =>
      _SmartTagSectionState();
}

class _SmartTagSectionState
    extends State<_SmartTagSection> {
  String _searchTerm = "";
  ...
  void _addTag(String tag) {
    ...
    final existing = widget.selectedTags.firstWhere(
          (t) =>
      t.toLowerCase() == tag.trim().toLowerCase(),
      orElse: () => "",
    );
    if (existing.isEmpty) {
  ...
  Widget build(BuildContext context) {
    ...
    widget.suggestions.where((s) {
      ...
      final isNotSelected =
      !widget.selectedTags.contains(s);
      return matchesSearch && isNotSelected;
    }).toList();

    final bool showCustomAdd = _searchTerm.isNotEmpty &&
        !widget.suggestions.any((s) =>
        s.toLowerCase() ==
            _searchTerm.toLowerCase()) &&
        !widget.selectedTags.any((t) =>
        t.toLowerCase() ==
            _searchTerm.toLowerCase());
          ...
          child: TextField(
            ...
            onTap: () {
              Future.delayed(
                  const Duration(milliseconds: 300),
                      () {
                    if (_inputFieldKey.currentContext !=
                        null) {
                      Scrollable.ensureVisible(
            ...
            decoration: InputDecoration(
              hintText:
              "Ajouter (ex: ${widget.suggestions.first})...",
              hintStyle: TextStyle(
...
class _FullScreenGalleryViewerState
    extends State<_FullScreenGalleryViewer> {
