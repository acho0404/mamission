...
class _ExplorePageState extends State<ExplorePage> with TickerProviderStateMixin {
  ...
  final ValueNotifier<double> _panelPosition = ValueNotifier(0.25);
  ...
  void _onFilterScroll() {
    ...
    if (max <= 0) {
      if (_showFilterScrollHint.value != false) {
        _showFilterScrollHint.value = false;
      }
      return;
    }

    final offset = position.pixels;
    final shouldShow = offset < max - 4;

    if (_showFilterScrollHint.value != shouldShow) {
      _showFilterScrollHint.value = shouldShow;
    }
  }

  Widget _buildFilterScrollHint() {
    return ValueListenableBuilder<bool>(
      valueListenable: _showFilterScrollHint,
